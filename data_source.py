...
class DataSource:
    ...
    def create_tables(self):
        commands = (
            ...
            """
                CREATE TABLE IF NOT EXIST reminders (
                    reminder_id serial PRIMARY KEY,
                    chat_id INT NOT NULL,
                    message VARCHAR(300) NOT NULL,
                    time TIMESTAMP NOT NULL,
                    fired BOLLEAN NOT NULL DEFAULT FALSE
                )
            """,
        )
    ...
    def get_all_reminders(self):
        ...
        try:
            conn = self.get_connection()
            cur = conn.cursor()
            cur.execute(SELECT_ALL_REMINDERS_STATMENT)
    ...
    def fire_reminder(self, reminder_id):
        ...
        try:
            conn = self.get_connection()
            cur = conn.cursor()
            cur.execute(FIRE_REMINDE_STATEMENT, (reminder_id))
            cur.close()
            conn.commit()
        except (Exception, psycopg2.DatabaseError) as error:
            logger.error(error)
